        ...
        return (
            <div style={{
                ...
                <div style={{
                    ...
                    height: "35px", 
                    visibility: "visible",
                    justifySelf: "end" }} ref={el => baseWallsRef.current[index] = el} key={uuidv4()}></div>
                <div style={{
                    backgroundColor: "rgba(49, 30, 20, 0.763)",
                    width: "60px",
                    height: "11px",
                    visibility: "visible",
                    gridColumnStart: "1",
                    gridColumnEnd: "4" }} ref={el => baseWallsRef.current[index] = el}></div>
            </div >
        )
    })
...
const animate = useCallback((shell, enemyTank, tank, setShell, spawnLeft, spawnRight, tankRotation) => {
    ...
    for (let index = 0; index < 2; index++) {
        let wallCoordinate = wallsRef.current[7].current[index].children[2].getBoundingClientRect()
        if (wallCoordinate.x < shellCoordinate.x + shellCoordinate.width &&
            wallCoordinate.x + wallCoordinate.width > shellCoordinate.x &&
            wallCoordinate.y < shellCoordinate.y + shellCoordinate.height &&
            wallCoordinate.height + wallCoordinate.y > shellCoordinate.y && wallsRef.current[7].current[index].children[2].style.visibility === "visible") {
            setShell(null)
            wallsRef.current[7].current[index].children[2].style.visibility = "hidden"
        }
    }
